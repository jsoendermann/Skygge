...
public class SkyggeFrame extends javax.swing.JFrame {
    ...
    private void checkForNewVersionAndUpdateMOTD() {
        Thread t = new Thread() {
            public void run() {
                String statusMessage = STATUS_BAR_MESSAGE_THANK_YOU;
                try {
                    ...
                    if (!newestVersion.equals(Skygge.VERSION)) {
                        statusMessage = STATUS_BAR_MESSAGE_UPDATE;
                    } else {
                        statusMessage = messageOfTheDay;
                    }
                } catch (IOException e) {
                ...
                EventQueue.invokeLater(new Runnable() {
                    public void run() {
                        statusBarLabel.setText(statusMessageFinalCopy);
                    }
                });
            }
        };
    ...
    private void loadDefaultSentence() {
        Thread t = new Thread() {
            public void run() {
                try {
                    sentenceAudioData = Utils.loadResourceIntoByteArray("/resources/animals2_001.wav");
